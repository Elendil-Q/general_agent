...
> **沙箱隔离的离线替代方案**：本指南的沙箱配置为 `LocalSandboxProvider`（宿主机直跑，见 §1.1）。若离线环境需要容器级沙箱隔离（AIO Sandbox），不必部署全量 Kubernetes——在目标机装一个**单机 k3s** 配合 provisioner 模式即可，完整步骤见 [offline-k3s-setup.md](offline-k3s-setup.md)。
