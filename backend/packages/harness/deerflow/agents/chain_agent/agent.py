...
def make_chain_agent(config: RunnableConfig, *, app_config: AppConfig) -> CompiledStateGraph:
    ...
    logger.info("Building chain graph %r (%d nodes)", chain.name, len(chain.nodes))
    graph = build_chain_graph(chain, app_config=resolved_app_config)
