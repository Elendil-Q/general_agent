...
- ``general-purpose`` allows ``ask_clarification``; ``bash`` keeps it restricted.
...
def test_bash_subagent_still_disallows_ask_clarification():
    """The bash subagent is an execution specialist; it should not clarify."""
    assert "ask_clarification" in BASH_AGENT_CONFIG.disallowed_tools
