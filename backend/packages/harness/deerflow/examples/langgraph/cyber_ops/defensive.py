"""Defensive-analysis subgraph for the ``cyber_ops`` workflow.

A single-LLM-node ``StateGraph`` that takes the user request as ``input`` and
returns a defensive-analysis conclusion as ``output``. Kept deliberately
...
# TODO: 按实际业务补充防御分析提示词。
SYSTEM_PROMPT = "You are a defensive analyst. Based on the user's request, produce a concise defensive-analysis conclusion."


class DefensiveState(ThreadState):
    """State schema for the defensive subgraph (extends the executor ThreadState)."""
...
def build_graph(*, model, tools, config) -> StateGraph:  # type: ignore[override]
    """Build the defensive-analysis subgraph (uncompiled).
    ...
    async def analyze(state: DefensiveState) -> dict:
    ...
    graph = StateGraph(DefensiveState)
