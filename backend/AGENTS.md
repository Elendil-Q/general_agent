...
**Interrupt + resume**: Because the subagent now has its own checkpointer + thread identity, a subagent tool/node may call `interrupt()`. `_aexecute`'s `astream(stream_mode="values")` loop captures `__interrupt__` on the final chunk and transitions `SubagentStatus` to `INTERRUPTED` (a new non-terminal, "stopped-but-resumable" status: `is_stopped` is true, `is_terminal` is false, so the result + executor stay resident and are NOT cleaned up). `task_tool` emits a `task_interrupted` SSE event (with `task_id`, `subagent_thread_id`, `interrupts`) and returns a pause message containing the resume handle so the lead agent's turn can end naturally. Cancel takes precedence over interrupt when both coincide. Resume is in-process (single Gateway session): `POST /api/threads/{parent_thread_id}/subagents/{task_id}/resume` (body `{"resume": ...}`) calls `resume_background_subagent(task_id, resume_value)`, which re-invokes the *same* cached `SubagentExecutor` + compiled agent + checkpointer via `Command(resume=...)` on the subagent's own `astream` — the lead-agent pipeline is never involved. The endpoint streams the resumed run's `task_*` events as SSE. A resumed run may itself interrupt again. The `SubagentExecutor` is kept resident in an in-process registry keyed by `task_id`; Gateway restart loses paused subagents (consistent with the in-memory `_background_tasks` model).
