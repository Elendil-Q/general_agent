...
- Extends `AgentState` with: `sandbox`, `thread_data`, `title`, `artifacts`, `todos`, `uploaded_files`, `viewed_images`, `subagents`
- Uses custom reducers: `merge_artifacts` (deduplicate), `merge_viewed_images` (merge/clear), `merge_subagents` (None keeps existing, otherwise snapshot replacement)
...
21. **SubagentContextMiddleware** - *(optional, if `subagent_enabled`)* Consolidated subagent-awareness middleware. `awrap_model_call` injects real-time subagent status (running/completed/interrupted/idle/failed) as a transient `SubagentStatusMessage` via `request.override(messages=...)` so the lead model sees subagent progress without calling `wait_for_tasks`. `abefore_model` persists a snapshot of the in-memory `agent_registry` into the `ThreadState.subagents` channel (its `state_schema` declares the channel; built by `subagents/state_mirror.py::build_subagents_snapshot`) — registry refs are mirrored verbatim (`TIMED_OUT`→`failed`), and entries that vanished from the registry are reconciled: IDLE→`completed`, RUNNING/PENDING/INTERRUPTED→`cancelled`, terminal kept; unchanged snapshots return `None` (no state write). This mirror is the frontend's restart/cold-open source of truth (the registry itself is process-local), delivered via `values` streaming like `todos`. `aafter_model` enforces both: (a) concurrent-subagent limit - truncates excess `task` tool calls to `MAX_CONCURRENT_SUBAGENTS`, and (b) pending-task guard - injects a `wait_for_tasks` reminder when the model tries to stop with PENDING/RUNNING subagents, forcing `tool_choice` after 3 reminders. Absorbs the former `SubagentLimitMiddleware` and `PendingTaskGuardMiddleware`.
...
26. **ClarificationMiddleware** - Intercepts `ask_clarification` tool calls (must be last). For structured interactions (`single_choice`/`multi_choice`/`text`) when the runtime context flag `clarification_interrupt_enabled` is set (web frontend only), calls `interrupt()` with a `ClarificationInterruptRequest` payload; the frontend renders a form modal and resumes via `Command(resume=...)`, and the agent continues the turn inline. For `free` interactions or when the flag is absent (IM channels, which can't resume), falls back to `Command(goto=END)` with a markdown `ToolMessage`. The interaction type is a tool input parameter (no second LLM classification). `clarification_interrupt_enabled` is whitelisted in `_CONTEXT_CONFIGURABLE_KEYS` (`services.py`).
...
| **Thread Runs** (`/api/threads/{id}/runs`) | `POST /` - create background run; `POST /stream` - create + SSE stream; `POST /wait` - create + block; `POST /regenerate/prepare` - prepare clean input + checkpoint metadata for regenerating the latest assistant answer; `GET /` - list runs; `GET /{rid}` - run details; `POST /{rid}/cancel` - cancel; `GET /{rid}/join` - join SSE; `GET /{rid}/messages` - paginated messages `{data, has_more}`; `GET /{rid}/events` - full event stream; `GET /../messages` - thread messages with feedback; `GET /../token-usage` - aggregate tokens; `GET /../subagents/{task_id}/messages` - persisted subagent conversation (`{task_id, thread_id, subagent_type, description, status, messages}`, 404 when no events and no mirror entry) |
...
**Concurrency**: `MAX_CONCURRENT_SUBAGENTS = 3` enforced by `SubagentContextMiddleware` (truncates excess `task` tool calls in `aafter_model`); default subagent timeout `subagents.timeout_seconds=1800` (30 min) and built-in `general-purpose` `max_turns=150` (raised from 100/15-min so deep-research subtasks stop hitting `GraphRecursionError` out of the box)
**Nesting** (subagent spawning subagents): Opt-in per subagent type via `allow_subagents: true` (YAML / `subagents.custom_agents` / `SubagentConfig`; default `false`). When allowed, the executor strips `task` from `disallowed_tools`, injects `nested_subagents_allowed` into the subagent's runtime context (`SubagentExecutor._apply_runtime_context`), and `task_tool` permits delegation only while `depth < MAX_SUBAGENT_DEPTH = 2` (lead→depth 1→depth 2 max; depth-2 subagents never get subagent tools). Each parent is limited to `MAX_CONCURRENT_SUBAGENTS` active children via `agent_registry.count_active_children(parent_task_id)`. Lineage is threaded end-to-end: `AgentRef.parent_task_id`/`depth`, every `task_*` event payload (injected by `SubagentExecutor._emit`), and the `ThreadState.subagents` mirror entry — the frontend uses `parent_task_id` to hide nested tasks from the activity panel. Nested `task_tool`/`follow_up`/`wait_for_tasks` runs skip `sse_bridge.register_writer` so they cannot clobber the lead run's stream writer.
**Flow**: `task()` and `follow_up()` are both fire-and-forget — they spawn/revive the subagent on the background scheduler and return immediately; `wait_for_tasks()` is the single blocking collector that polls the `agent_registry` mirror (5s) until terminal or IDLE and returns the result payload, with SSE events streaming progress in between
**Events**: `task_started`, `task_running`, `task_completed`/`task_failed`/`task_timed_out`/`task_interrupted`; keep_alive lifecycle adds `task_idle` (parked, follow-up-able), `task_revived` (resumed by `follow_up`), and `task_expired` (TTL reaped → transitions to COMPLETED). All payloads carry `parent_task_id`/`depth` lineage (`None`/`1` for lead-spawned tasks). The structured `ToolMessage.additional_kwargs.subagent_status` contract (`subagents/status_contract.py`, fixture `contracts/subagent_status_contract.json`) accepts `idle`/`interrupted` as non-terminal stamps alongside the terminal values.
**Message persistence** (subagent conversation view): The executor emits `subagent:message` bus events (full AI/Tool message `model_dump`) from all three astream capture blocks; `subagents/message_persistence.py` (`SubagentMessagePersister`) bridges them onto the main loop and into the run event store as `category="subagent_message"` events keyed by `metadata.task_id` (bound in the gateway lifespan, `app/gateway/deps.py`). `RunEventStore.list_subagent_messages(thread_id, task_id)` (memory/jsonl/db backends, seq ascending) backs `GET /api/threads/{thread_id}/subagents/{task_id}/messages` (`app/gateway/routers/thread_runs.py`) — the data source for the frontend's read-only subagent conversation view.
...
**Checkpointer isolation**: Each subagent owns its own async-capable, loop-safe `InMemorySaver` (compiled with `checkpointer=self._checkpointer`) — never the parent run's sync checkpointer nor the Gateway's main-loop-bound async saver, both of which break on the isolated subagent event loop. The subagent derives an independent `subagent_thread_id` (`subagent::{parent_thread_id}::{task_id}`) that keys its own checkpoint, while the parent `thread_id` stays in `runtime.context` so sandbox/file tools keep writing to the parent workspace.
**Interrupt + resume** (Route 甲 — lead agent waits): Because the subagent has its own checkpointer + thread identity, a subagent tool/node may call `interrupt()` (subagents now carry `ClarificationMiddleware` last, like the lead agent, and `clarification_interrupt_enabled` is forwarded from the parent runtime context). `_aexecute`'s `astream(stream_mode="values")` loop captures `__interrupt__` on the final chunk and transitions `SubagentStatus` to `INTERRUPTED` (a non-terminal, "stopped-but-resumable" status: `is_stopped` is true, `is_terminal` is false, so the result + executor stay resident and are NOT cleaned up). The executor transitions both the `SubagentResult` holder and the `agent_registry` mirror to `INTERRUPTED` (in `_aexecute`/`_acontinue`/`_aresume` alike) and emits a `task_interrupted` SSE event (with `task_id`, `subagent_thread_id`, `interrupts`) **once per pause**. `wait_for_tasks` does **not** return for INTERRUPTED — it keeps polling with its timeout suspended, so the lead run stays open (blocked in the tool) while the user answers. The subagent is resumed via `POST /api/threads/{parent_thread_id}/subagents/{task_id}/resume` (body `{"resume": ...}`) → `resume_background_subagent(task_id, resume_value)`, which re-invokes the *same* cached `SubagentExecutor` + compiled agent + checkpointer via `Command(resume=...)` on the subagent's own `astream`. When the subagent reaches `COMPLETED` (or IDLE under `keep_alive` — `_aresume` mirrors the same keep_alive parking branch as `_aexecute`/`_acontinue`, including the registry-mirror update), `wait_for_tasks` returns the result payload and the lead model continues **in the same turn** (no continuation-run mechanism). Cancel takes precedence over interrupt; a parent-cancel while INTERRUPTED cancels the paused subagent (the parallel cohort's "全部取消" on disconnect/thread-switch). The execution-timeout countdown is suspended while INTERRUPTED (only RUNNING polls advance it), so a slow human reply never trips it. The endpoint's own SSE stream is redundant under Route 甲 — the frontend watches the still-open lead run stream for the subsequent `task_running`/`task_completed` events; the resume POST is fire-and-forget. A resumed run may itself interrupt again. Accepted trade-off: the lead's blocked-in-tool state is not checkpointed, so a Gateway restart during a paused subagent loses the turn (consistent with normal tool-call in-flight state loss; subagents keep `InMemorySaver`).

**Follow-up** (`follow_up` tool): fire-and-forget like `task` — validates the subagent is IDLE, cancels the TTL, registers the SSE writer, then calls `SubagentExecutor.continue_async`, which revives the REGISTERED `SubagentResult` holder via `try_revive()` (IDLE→RUNNING; clears `idle_since`/`completed_at`/`error`/interrupt fields and the cancel event) and schedules `_acontinue` on the scheduler pool. The tool returns immediately and its docstring directs the model to `wait_for_tasks`. Because the continuation mutates the registered holder (never a fresh unregistered one — that earlier bug left the registry holder IDLE and caused 409 "not interrupted" on resume), an interrupt during a follow-up is resumable through the same resume endpoint, and `wait_for_tasks`/middleware/frontend all observe the mirror flip RUNNING → INTERRUPTED → COMPLETED/IDLE.

**Workflow subagents** (strict-flow alternative to `create_agent`): A subagent config may set `workflow: "module.path:object"` — a `module:object` reference resolved by the same `resolve_variable` loader used for `config.tools[].use` (e.g. `deerflow.workflows.research_review:build_graph`). When set, `SubagentExecutor._create_agent` dispatches to `_create_workflow_agent` instead of `create_agent`: it resolves the parent model + policy-filtered tools, calls `factory(*, model, tools, config) -> StateGraph`, and compiles the returned **uncompiled** `StateGraph` (over a `ThreadState`-compatible schema) with the executor's own `InMemorySaver`. Register them exactly like custom `create_agent` subagents — in `config.yaml`'s `subagents.custom_agents` (set `workflow`, omit `system_prompt`/`skills`/`tools`-filtering, which the workflow owns) or as a `SubagentConfig` in `BUILTIN_SUBAGENTS`. The executor's outer machinery (background scheduler, `SubagentStatus`, timeout, cancel, HITL via native `interrupt()` → `Command(resume=...)`, `_extract_final_result` reading the terminal `AIMessage`) is workflow-agnostic and runs unchanged — a workflow subagent just needs to be a `CompiledStateGraph` that speaks the messages/sandbox/thread_data state protocol. Key boundary: the shared `build_subagent_runtime_middlewares` chain (sandbox lifecycle, guardrails, audit, deferred-MCP filter) does **not** auto-apply — langgraph `StateGraph` has no `add_middleware`, and `AgentMiddleware` is a `create_agent`-only concept. The graph owns its nodes/edges; sandbox/MCP tools still work when routed through a `ToolNode` (the `ToolRuntime` is injected from the `context` the executor passes to `astream`, and `sandbox`/`thread_data` flow from the graph state — forward the full state to `ToolNode.ainvoke`, not just `messages`, or the tools see an empty workspace). Example: `packages/harness/deerflow/workflows/research_review.py` (research step calls `read_file`/`write_file` via `ToolNode`). Tests: `tests/test_subagent_workflow.py`, `tests/test_workflow_sandbox_tools.py`.

### Chain Pipelines (`packages/harness/deerflow/chains/`, `packages/harness/deerflow/agents/chain_agent/`)

A *chain* is a YAML-configured DAG of subagent invocations, triggered by the `/chain:<name>` slash command. Each chain is a flat `chains/{public,custom}/<name>.yaml` file; the filename is the chain name (hyphen-case). The Gateway's `start_run` detects the `/chain:<name>` prefix in the user message, persists `chain_name` to thread metadata, and swaps the agent factory from `make_lead_agent` to `make_chain_agent` for that turn - the chain runs as the **top-level graph** (peer to the lead agent), so a subagent's native `interrupt()` inside a node gets real HITL via the thread checkpointer. `depends_on` maps directly to graph edges: LangGraph's Pregel engine handles parallel fan-out (multiple roots from `START`) and barrier/join (a node with multiple `depends_on` waits for all of them) natively. Each node invokes one subagent to completion (inline `ainvoke`, no background executor), renders its prompt (`{input}` + `{node_outputs.<name>}`), and writes its result into the `node_outputs` channel. Terminal nodes (nothing depends on them) also append a final `AIMessage` to `messages` so the chain's answer streams and persists.

`ChainState.node_outputs` uses a `merge_node_outputs` reducer (empty dict = explicit clear, otherwise merge) so parallel root nodes return *incremental* updates (`{node_name: result}`) instead of clobbering each other. A fresh `/chain:` run seeds `node_outputs={}` in the graph input to wipe any residue from a previous turn.

**Chain resume** (`/chain-resume:<name>`): each chain run writes a progress file at `{base_dir}/users/{user_id}/threads/{thread_id}/chains/{chain_name}/progress.json` (thread-isolated, outside the sandbox user-data tree) recording per-node status/result, the chain spec hash (`compute_chain_hash`), the original user input, and `terminal_nodes`. `ChainProgressStore` (sync impl + `threading.Lock`, callers wrap in `asyncio.to_thread`) owns create/load/update/start_resume/mark_status/mark_completed_if_done/list_for_thread/delete. On a force-cancel (Stop button `action=interrupt`/`rollback`) or process crash, the run-task `add_done_callback` finalises the file: cancel -> `interrupted` (kept recoverable even on rollback, which wipes the checkpoint but not the progress file), real error -> `failed`, success -> `mark_completed_if_done` (a HITL `interrupt()` pause surfaces as `success` with an unfinished graph and is left non-completed). `/chain-resume:<name>` reloads the progress, verifies the chain hash has not drifted (409 on mismatch), calls `start_resume`, and injects `chain_resume={"completed_nodes": {...}, "input": ...}` into the run config - `make_chain_agent` forwards it to `build_chain_graph`/`make_subagent_node`, which short-circuit completed nodes (replay cached result, skip the subagent, no duplicate terminal `AIMessage`) and use `resume_input` when the resume run has no new HumanMessage. The progress REST endpoints (`GET/DELETE /api/threads/{thread_id}/chains[/{chain_name}]/progress`) report a `resumable` flag: not completed, no in-flight run, and the thread checkpoint has no `__interrupt__` (HITL pauses resume via `Command(resume=...)`, not `/chain-resume:`). Frontend autocomplete surfaces `/chain-resume:<name>` for resumable chains. Tests: `tests/test_chain_*`, `tests/test_thread_chains_progress_router.py`.
...
3. **Built-in tools**:
   ...
   - `ask_clarification` - Request clarification (intercepted by ClarificationMiddleware → structured interrupt for single_choice/multi_choice/text, or goto=END for free/IM)
...
- `subagents` - Subagent runtime config (timeouts, max_turns, per-agent overrides including keep_alive, custom_agents). The `task` tool is bound at runtime via `subagent_enabled` in `config.configurable` (the frontend derives it from the selected mode preset), not from a config switch — there is no `subagents.enabled` field
