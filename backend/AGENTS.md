...
**Events**: `task_started`, `task_running`, `task_completed`/`task_failed`/`task_timed_out`/`task_interrupted`; keep_alive lifecycle adds `task_idle` (parked, follow-up-able), `task_revived` (resumed by `follow_up`), and `task_expired` (TTL reaped → transitions to COMPLETED). The structured `ToolMessage.additional_kwargs.subagent_status` contract (`subagents/status_contract.py`, fixture `contracts/subagent_status_contract.json`) accepts `idle`/`interrupted` as non-terminal stamps alongside the terminal values.
