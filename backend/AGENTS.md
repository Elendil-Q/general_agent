...
LangSmith, Langfuse, and Arize Phoenix are supported. The wiring lives in two layers:
...
- `phoenix.py::register_phoenix_tracing()` — modern Phoenix (`arize-phoenix-otel` ≥0.16) is **process-global OpenTelemetry instrumentation**, not a per-run callback. The `phoenix` branch in `build_tracing_callbacks()` calls this idempotent registrar (thread-lock + module flag) and appends **no** callback — it lazily imports `phoenix.otel.register`, sets the global `TracerProvider` and auto-instruments the installed `openinference-instrumentation-langchain` tracer, so every LangChain/LangGraph/LLM/tool call after the first graph-root invocation is traced automatically (including standalone model calls like `MemoryUpdater`, with no double-tracing). Coexists with the LangSmith/Langfuse callback handlers. Requires the optional extra `deerflow-harness[phoenix]`; a helpful `RuntimeError` with the install hint is raised if it's missing.

**Trace-attribute injection points**: both `runtime/runs/worker.py::run_agent` (gateway path) and `client.py::DeerFlowClient.stream` (embedded path) merge the metadata into `config["metadata"]` right before constructing the graph. `subagents/executor.py::_aexecute` does the same for every subagent run so subagent traces group under the parent thread's session card (carrying the parent `thread_id` → `langfuse_session_id`, the user_id captured at `task_tool` → `langfuse_user_id`, and a `subagent:<normalized-name>` trace name). Caller-supplied keys win via `setdefault`, so an external `session_id` override is preserved. The shared `inject_trace_metadata()` helper fans out to both Langfuse and Phoenix so the injection points can't drift. Field mapping:
...
Returns `{}` when Langfuse is not in the enabled providers — LangSmith-only deployments are unaffected. Set `DEER_FLOW_ENV` (or `ENVIRONMENT`) to tag traces by deployment environment. Tests live in `tests/test_tracing_factory.py`, `tests/test_tracing_metadata.py`, `tests/test_tracing_phoenix.py`, `tests/test_worker_langfuse_metadata.py`, `tests/test_client_langfuse_metadata.py`, and `tests/test_subagent_executor.py::TestSubagentTracingWiring`.

**Phoenix span naming / kind**: the OpenInference LangChain tracer names each span after the LangChain run name and derives `openinference.span.kind` from a `"agent" in run.name.lower()` heuristic (falling back to the run type, so LangGraph graph roots default to `CHAIN`). To keep subagent root spans readable and correctly classified, `subagents/builder.py::build_subagent_agent` compiles every `create_agent` subagent graph with `name=f"subagent:<normalized-name>"` (lowercase, `_`→`-`, falling back to `"subagent"`) — the `"subagent:"` prefix contains "agent", so Phoenix shows `subagent:general-purpose` with kind `AGENT` instead of the `LangGraph`/`CHAIN` default. Workflow subagents were already covered the same way (`subagent-workflow:<name>` at compile time). Regression tests: `tests/test_subagent_builder.py::TestSubagentGraphNaming`.

**Phoenix session / user grouping**: the OpenInference LangChain tracer lifts `session_id` / `thread_id` from `RunnableConfig.metadata` onto `session.id`, so `build_phoenix_trace_metadata()` injects `thread_id` (+ `session_id`, `user_id`) at the same injection points above and multi-turn traces group into Phoenix sessions. `user.id` is not read from metadata, so `phoenix_span_context(session_id=..., user_id=...)` (a no-op wrapper around OpenInference `using_attributes`) is applied around the graph invocations in `runtime/runs/worker.py::run_agent`, `client.py::DeerFlowClient.stream`, and both `subagents/executor.py::_aexecute` / `_aresume` (inside the background execution thread, since OTel context doesn't cross threads). Phoenix env vars: `PHOENIX_TRACING` (enable), `PHOENIX_COLLECTOR_ENDPOINT` (default `http://localhost:6006/v1/traces`), `PHOENIX_PROJECT_NAME` (default `deer-flow`), `PHOENIX_HEADERS` (optional JSON object).
