...
- **`app/`** - Next.js App Router. Routes include `/` (landing), `/workspace/chats/[thread_id]` (chat), `/workspace/agents/[agent_name]` and `/workspace/agents/new` (custom agents), `/workspace/subagents` (custom subagent designer: gallery + new/edit pages), `/blog/…`, the `(auth)/{login,setup,auth/callback}` flow, `/[lang]/docs/…`, and `/api/…` route handlers (e.g. `/api/memory`).
...
- **`core/`** - Business logic, the heart of the app. Domains include `threads/` (creation, streaming, state), `api/` (LangGraph client singleton), `agents/` (custom agents), `subagents/` (custom subagent type designer: CRUD + tool/skill catalogs), `auth/` (authentication), `artifacts/`, `channels/` (IM connections), `i18n/` (en-US, zh-CN), `settings/`, `memory/`, `skills/`, `messages/`, `mcp/`, `models/`, `effort/` (config-driven effort presets fetched from `GET /api/efforts/config`), `suggestions/`, `tasks/`, `todos/`, `tools/`, `config/`, `notification/`, `blog/`, plus rendering helpers (`rehype/`, `streamdown/`) and `utils/`.
...
2. Stream events update thread state (messages, artifacts, todos). Subagent status rides two channels: live `custom` SSE events (`task_started`/`task_idle`/… → `core/threads/custom-events.ts`) as real-time increments, and the persisted `ThreadState.subagents` mirror channel (`values` mode → `useSubagentMirrorSync` in `core/tasks/context.tsx`, pure helpers in `core/tasks/mirror.ts`) as the restart/cold-open baseline — with a local IDLE-TTL timer (`idle_expires_at`) covering the `task_expired` event that the backend cannot deliver once the lead run's stream has closed. Nested subagents (depth 2, spawned by a subagent whose type has `allow_subagents: true`) carry `parent_task_id` in both channels: `handleCustomEvent` swallows their `task_*` events, the mirror stores them with lineage, and `ActivityPanel` filters them out via `isLeadSubtask` (`core/tasks/nested.ts`) — their only UI surface is a transient delegation hint on the parent's `SubtaskCard`, derived from `task` tool calls in the parent's `latestMessage` (`getNestedDelegations`)
3. The read-only subagent conversation view at `/workspace/chats/[thread_id]/subagents/[task_id]` (entered by clicking a task in `SubagentList`) loads persisted history via `useSubagentMessages` (`core/tasks/subagent-messages.ts`, backed by `GET /api/threads/{thread_id}/subagents/{task_id}/messages`) and merges the live `task_progress` snapshot from the subtask store by message id (`mergeSubagentLiveMessage`), freezing once the task is terminal. Rendering reuses the main chat's grouping pipeline via `SubagentMessageList` (`components/workspace/messages/subagent-message-list.tsx`), statically (no streaming). Because the chat page swaps the URL of a newly created thread via the native History API (leaving `useParams()` stale at `"new"`), both `SubagentList` and the subagent page resolve the real thread id from the pathname via `useResolvedThreadId` (`core/threads/thread-id.ts`). The same view works for nested tasks by URL (the backend messages endpoint is keyed by `(thread_id, task_id)` only), but no UI links to it
